...
============================================= *//* =============================================
   ...
   ║  1. Curseur custom + spotlight            ║
   ║  2. Canvas particules (classe Pt)         ║
   ║  3. Navigation onglets (tabs)             ║
   ║  4. Navbar scroll                         ║
   ║  5. Menu mobile (burger)                  ║
   ║  6. Reveal animations au scroll           ║
   ║  7. Card tilt 3D                          ║
   ...
   ║  12. Compteur animé                       ║
   ║  13. Slideshow images                     ║
   ║  14. Zoom image (lightbox générique)      ║
   ║  15. Lightbox photo profil                ║
   ║  16. Popups description projet            ║
   ║  17. Sélecteur de CV                      ║
   ║  18. Easter egg console                   ║
   ║  19. Easter egg Git Dumber                ║
   ║  20. Lightbox galerie graphisme           ║
   ╚═══════════════════════════════════════════╝
   
   Convention : gs(), st(), tm(), cm() = fonctions courtes
   pour les handlers inline HTML.
============================================= */

/* =============================================
   0. EMAIL — Obfuscation anti-scraping
   
   L'adresse est reconstruite en JS au runtime.
   Elle n'est jamais présente en clair dans le HTML.
============================================= */
(function () {
...
/* =============================================
   ...
   mx/my : position exacte de la souris
   rx/ry : position interpolée (lerp à 10%) → effet lag naturel
   
   requestAnimationFrame : boucle ~60fps synchronisée au refresh écran.
============================================= */
const cur = document.getElementById("cur");
const cring = document.getElementById("cring");
const spl = document.getElementById("spl");
let mx = 0, my = 0, rx = 0, ry = 0, mouseX = 0, mouseY = 0;

document.addEventListener("mousemove", (e) => {
  mx = e.clientX;
  my = e.clientY;
  if (cur) { cur.style.left = mx + "px"; cur.style.top = my + "px"; }
  if (spl) { spl.style.setProperty("--sx", mx + "px"); spl.style.setProperty("--sy", my + "px"); }
  mouseX = mx;
  mouseY = my;
});
...
document.querySelectorAll("a,button,.card,.pill,.sk-card,.gal-it,.toolp,.tech-item,.stat,.dc,.wow-skill-card,.cv-sel-btn,.proj-desc-btn,.img-zoomable,.wow-photo-wrap").forEach((el) => {
  el.addEventListener("mouseenter", () => { cur && cur.classList.add("hov"); cring && cring.classList.add("hov"); });
  el.addEventListener("mouseleave", () => { cur && cur.classList.remove("hov"); cring && cring.classList.remove("hov"); });
});

/* =============================================
   2. CANVAS PARTICULES
   
   Classe Pt (Point) : chaque instance a son propre état.
   MOUSE_RADIUS/MOUSE_FORCE : les particules proches sont repoussées.
   ctx.clearRect() : efface et redessine tout à chaque frame.
============================================= */
const canvas = document.getElementById("pc");
const ctx = canvas ? canvas.getContext("2d") : null;
let W, H, pts = [];
const MOUSE_RADIUS = 160, MOUSE_FORCE = 1.8;

function rsz() { W = canvas.width = window.innerWidth; H = canvas.height = window.innerHeight; }
if (canvas) { rsz(); window.addEventListener("resize", rsz); }
...
class Pt {
  ...
  upd() {
    ...
    if (spd > 2.5) { this.vx *= 2.5 / spd; this.vy *= 2.5 / spd; }
    this.vx *= 0.99; this.vy *= 0.99;
    this.x += this.vx; this.y += this.vy;
    this.life++;
    if (this.x < -10 || this.x > W + 10 || this.y < -10 || this.y > H + 10 || this.life > this.ml) this.res();
  }
  drw() {
    const f = this.life < 60 ? this.life / 60 : this.life > this.ml - 60 ? (this.ml - this.life) / 60 : 1;
    ctx.beginPath();
...
if (ctx) { for (let i = 0; i < 130; i++) pts.push(new Pt()); }
  ...
  for (let i = 0; i < pts.length; i++) {
    for (let j = i + 1; j < pts.length; j++) {
      ...
      if (d < md) {
        ...
        ctx.strokeStyle = "#8A9463"; ctx.globalAlpha = (1 - d / md) * 0.055; ctx.lineWidth = 0.5; ctx.stroke();
      }
    }
  }
}
...
/* =============================================
   ...
   data-tab="frontend" → attribut HTML5 custom pour cibler
   le bon bouton sans dépendre de l'ordre dans le DOM.
   setTimeout(rvl, 60) : le panneau doit être visible
   (display:block) avant que l'IO ne calcule sa position.
============================================= */
function st(id) {
...
/* =============================================
   ...
   classList.toggle(class, condition) : ajoute ou enlève selon bool.
   --nav-h : variable CSS mise à jour en JS pour décaler les sections.
============================================= */
function gs(id) {
...
/* =============================================
   ...
   tm() = "toggle menu" / cm() = "close menu"
   3 <span> animés en X via transform + opacity.
============================================= */
let menuOpen = false;
function tm() {
  menuOpen = !menuOpen;
...
function cm() { if (menuOpen) tm(); }

/* =============================================
   6. REVEAL ON SCROLL
   
   getBoundingClientRect().top < window.innerHeight - 50 :
   déclenche l'animation quand l'élément est à 50px du bas.
   { passive: true } : le navigateur sait qu'on ne bloque pas le scroll.
============================================= */
function rvl() {
...
/* =============================================
   ...
   Effet parallax 3D : rotate selon la position relative de la souris.
   perspective(1000px) = profondeur de la scène 3D.
   Math.abs(dx) < 1.8 : seulement si la souris est proche de la card.
============================================= */
document.addEventListener("mousemove", (e) => {
...
/* =============================================
   ...
   Character splitting : chaque lettre dans un <span class="char">
   avec animation-delay progressif → effet machine à écrire visuel.
   Note : en prod, ajouter aria-label sur le parent pour l'accessibilité.
============================================= */
function splitChars() {
  const el1 = document.querySelector(".hero-name .line1");
  if (el1) {
    el1.innerHTML = el1.textContent.split("").map((c, i) =>
      `<span class="char" style="animation-delay:${i * 0.04 + 0.3}s">${c === " " ? "&nbsp;" : c}</span>`
    ).join("");
  }
  const el2 = document.querySelector(".hero-name .line2");
  if (el2) {
    el2.innerHTML = el2.textContent.split("").map((c, i) =>
...
/* =============================================
   ...
   Fermeture : e.target === e.currentTarget vérifie que c'est le fond
   qui a été cliqué, pas la modale elle-même.
   body.style.overflow = "hidden" → bloque le scroll de fond.
   Escape = ferme toutes les modales (accessibilité clavier).
============================================= */
...
document.addEventListener("keydown", (e) => {
  if (e.key === "Escape") { closeModal(null); closeZoom(); closeAllDescPopups(); closePhotoZoom(); }
});

/* =============================================
   10. BLUR QUOTE EFFECT
   
   Chaque <span class="bw"> a des data-* lus en JS (valeurs flottantes).
   IntersectionObserver déclenche animate() à l'entrée dans le viewport.
   Boucle infinie avec reset + relance après 3.5s de pause.
============================================= */
(() => {
  ...
  let isAnimating = false, restartTimeout = null;

  function animate() {
    if (isAnimating) return;
    isAnimating = true;
    let maxDelay = 0, maxDuration = 0;
    words.forEach((word) => {
      ...
      setTimeout(() => { word.classList.add("animate"); word.style.filter = "blur(0px)"; word.style.opacity = "1"; }, 30);
    });
    if (cite) {
    ...
    restartTimeout = setTimeout(() => {
      ...
      if (cite) { cite.style.transition = "opacity .8s ease-out"; cite.classList.remove("animate"); }
      setTimeout(() => { isAnimating = false; animate(); }, 3500);
    }, totalTime + 500);
  }

  quoteEl.addEventListener("click", () => {
    clearTimeout(restartTimeout);
    words.forEach((w) => { w.classList.remove("animate"); w.style.filter = `blur(${w.dataset.blur || 4}px)`; w.style.opacity = "0"; });
  ...
  const obs = new IntersectionObserver((entries) => {
    entries.forEach((e) => { if (e.isIntersecting) { setTimeout(animate, 400); obs.disconnect(); } });
  }, { threshold: 0.5 });
...
/* =============================================
   ...
   Lignes mises à jour v3.1 : orientées UI Developer.
   Délai variable 55-90ms → simule la frappe humaine.
   Boucle infinie avec relance après 4.5s de pause.
============================================= */
(function () {
  ...
  const lines = [
    { cls: "comment",              txt: "// Sébastien Maurice — UI Developer 2026" },
    { cls: "keyword",              txt: "const dev = {" },
    { cls: "indent string",        txt: "  role: 'Front-End / UI Developer'," },
    { cls: "indent string",        txt: "  stack: ['HTML','CSS','JS','Svelte']," },
    { cls: "indent string",        txt: "  focus: ['interfaces','perf','a11y']," },
    { cls: "indent func",          txt: "  build: () => cleanUI()," },
    { cls: "keyword",              txt: "};" },
    { cls: "",                     txt: "" },
    { cls: "func",                 txt: "function cleanUI(spec) {" },
    { cls: "indent keyword",       txt: "  if (!spec.semantic)" },
    { cls: "indent indent string dramatic", txt: "    throw new Error('Fix HTML first.');" },
    { cls: "indent keyword",       txt: "  return spec.ship();" },
    { cls: "keyword",              txt: "}" },
  ];
  let lineIndex = 0, charIndex = 0, lineEl, timer;
  function type() {
  ...
  function start() {
    clearTimeout(timer); el.innerHTML = ""; el.appendChild(cursor);
...
/* =============================================
   ...
   dataset.counted = "1" : flag pour ne pas relancer.
   setInterval avec 60 étapes sur 1200ms = animation fluide.
   { passive: true } → scroll optimisé mobile.
============================================= */
function countUp() {
...
/* =============================================
   ...
   Map() pour stocker les timers par conteneur.
   startSlideshow/stopSlideshow au hover pour ne pas
   distraire l'utilisateur qui lit le contenu.
============================================= */
...
function startSlideshow(container) {
  ...
  const slides = container.querySelectorAll(".slide");
  if (slides.length < 2) return;
  const tid = setInterval(() => { current = (current + 1) % slides.length; goToSlide(container, current); }, 3000);
...
/* =============================================
   ...
   Créée dynamiquement en JS — pas dans le HTML.
   Navigation clavier (← →) et swipe tactile (diff > 50px).
   (index - 1 + length) % length → boucle sans négatifs JS.
============================================= */
let zoomImages = [], zoomIndex = 0;
...
function openZoom(images, startIndex) {
  zoomImages = images; zoomIndex = startIndex || 0;
  updateZoomImg();
...
function updateZoomImg() {
  ...
  zoomCounter.textContent = zoomImages.length > 1 ? `${zoomIndex + 1} / ${zoomImages.length}` : "";
  document.getElementById("imgZoomPrev").style.visibility = zoomImages.length > 1 ? "visible" : "hidden";
  document.getElementById("imgZoomNext").style.visibility = zoomImages.length > 1 ? "visible" : "hidden";
}
function zoomPrev() { zoomIndex = (zoomIndex - 1 + zoomImages.length) % zoomImages.length; updateZoomImg(); }
function zoomNext() { zoomIndex = (zoomIndex + 1) % zoomImages.length; updateZoomImg(); }
document.getElementById("imgZoomClose").addEventListener("click", closeZoom);
document.getElementById("imgZoomPrev").addEventListener("click", (e) => { e.stopPropagation(); zoomPrev(); });
document.getElementById("imgZoomNext").addEventListener("click", (e) => { e.stopPropagation(); zoomNext(); });
...
zoomOverlay.addEventListener("touchend", (e) => {
  ...
  if (Math.abs(diff) > 50) { diff > 0 ? zoomNext() : zoomPrev(); }
}, { passive: true });

function attachZoom() {
  document.querySelectorAll(".dc-slideshow,.modal-slideshow").forEach((container) => {
    const imgs = Array.from(container.querySelectorAll(".slide img"));
    imgs.forEach((img, idx) => {
      img.style.cursor = "zoom-in";
      img.addEventListener("click", (e) => { e.stopPropagation(); openZoom(imgs.map((i) => i.src), idx); });
    });
  });
  document.querySelectorAll(".cimg img,.dc-img img,.gal-it img").forEach((img) => {
    if (img.closest(".dc-slideshow") || img.closest(".modal-slideshow")) return;
    img.style.cursor = "zoom-in";
    img.addEventListener("click", (e) => { e.stopPropagation(); openZoom([img.src], 0); });
  });
}
attachZoom();
...
/* =============================================
   ...
   Textes repositionnés v3.1 : orientés Front-End / UI.
   positionPopup() : calcule la position optimale pour rester
   dans le viewport (evite les débordements droite/bas).
============================================= */
const projectDescriptions = {
  "cine-delices": {
    title: "Ciné <em>Délices</em>",
    tag: "Lead Front-End · SPA Svelte · Design System · API TMDB",
    desc: "L'idée : associer chaque film culte à une recette inspirée de son univers. Avant d'écrire une ligne de code, j'ai posé le design system complet sous Figma — palette, typographies, composants. SPA Svelte, appels API TMDB avec gestion des erreurs et fallbacks, carrousels natifs CSS/JS, animations sans librairie. Premier projet full-stack où j'ai géré seul le front de bout en bout.",
    features: [
      "Design system Figma complet avant le code",
      "SPA Svelte avec routing côté client",
      "Appels API TMDB + gestion erreurs et fallbacks",
      "Carrousels et animations CSS/JS natifs (sans librairie)",
      "Responsive mobile-first, testé iOS et Android",
      "Palette or/noir/rouge — typographies cinématographiques",
    ],
    chips: [
      { label: "Svelte", cls: "fe" }, { label: "HTML5", cls: "fe" }, { label: "CSS3", cls: "fe" },
      { label: "JavaScript ES6+", cls: "fe" }, { label: "Node.js", cls: "be" }, { label: "Express", cls: "be" },
      { label: "PostgreSQL", cls: "tool" }, { label: "API TMDB", cls: "tool" }, { label: "Figma", cls: "tool" },
    ],
  },
  "dev-ndumber": {
    title: "Dev <em>N'Dumber</em>",
    tag: "Lead Front-End · Git collaboratif · Projet O'Clock",
    desc: "Projet de fin de formation à 4 devs. Rôle : lead front-end. J'ai posé la charte graphique, structuré les vues HTML/CSS et géré le workflow Git en équipe — branches, PR, merge conflicts résolus en live. Ce projet m'a appris autant sur la coordination que sur le code.",
    features: [
      ...
      "Livraison en 2 semaines — méthodologie Agile",
    ],
    chips: [
      { label: "HTML5", cls: "fe" }, { label: "CSS3", cls: "fe" }, { label: "JavaScript ES6+", cls: "fe" },
      { label: "Node.js", cls: "be" }, { label: "Express", cls: "be" },
      { label: "PostgreSQL", cls: "tool" }, { label: "Git / GitHub", cls: "tool" },
    ],
  },
  "presents-angel": {
    title: "Les Présents <em>d'Ange'L</em>",
    tag: "Audit UX · Intégration UI · E-commerce bijoux",
    desc: "Audit UX et refonte des frictions d'interface pour une boutique bijoux. Hiérarchie visuelle des fiches produit retravaillée, CTA repositionné, galerie optimisée. Corrections responsive validées sur iOS et Android.",
    features: [
      ...
      "Correction responsive mobile — tests iOS/Android",
      "Cohérence de marque : couleurs et typographies harmonisées",
    ],
    chips: [
      { label: "HTML5", cls: "fe" }, { label: "CSS3", cls: "fe" },
      { label: "JavaScript", cls: "fe" }, { label: "Responsive Design", cls: "tool" },
    ],
  },
  palimpseste: {
    title: "Palimpseste <em>Urbain</em>",
    tag: "Architecture · WordPress · Migration OVH",
    desc: "Refonte WordPress pour un cabinet d'architecture et urbanisme. Plusieurs directions webdesign proposées, migration OVH, intégration Elementor. Structure pensée pour l'autonomie client — il peut maintenir son site sans m'appeler.",
    features: [
    ...
    chips: [
      ...
      { label: "SEO", cls: "tool" }, { label: "Performance", cls: "tool" }, { label: "OVH Migration", cls: "tool" },
    ],
  },
  "petit-chateau": {
    title: "Petit Château <em>Vercourt</em>",
    tag: "Tourisme · WordPress · SEO Local",
    desc: "Site vitrine touristique avec système de réservation. Back-office simplifié pour gestion en autonomie totale. SEO local ciblé, images optimisées pour connexions rurales. Avis client 5 étoiles.",
    features: [
      "Design élégant adapté tourisme rural",
      "Back-office simplifié pour autonomie client",
      "SEO local : mots-clés géolocalisés + Google My Business",
      "Images optimisées pour connexions limitées",
      "Système de réservation intégré",
    ],
    chips: [
      { label: "WordPress", cls: "fe" }, { label: "Divi", cls: "fe" },
      { label: "SEO Local", cls: "tool" }, { label: "Performance", cls: "tool" },
    ],
  },
  gsti62: {
    title: "<em>GSTI62</em>",
    tag: "Industrie · B2B · Migration Wix → WordPress",
    desc: "Migration complète Wix → WordPress, fusion de deux entités industrielles, webdesign sur mesure B2B, système de devis WooCommerce. Formation client au back-office — investissement qui réduit les demandes de support.",
    features: [
    ...
    chips: [
      { label: "WordPress", cls: "fe" }, { label: "Elementor", cls: "fe" },
      { label: "WooCommerce", cls: "fe" }, { label: "SEO", cls: "tool" }, { label: "Migration Wix", cls: "tool" },
    ],
  },
  alternativ: {
    title: "Alternativ' <em>Uniforme</em>",
    tag: "E-commerce · Vêtements professionnels",
    desc: "E-commerce vêtements professionnels. Personnalisation produit sur mesure, WooCommerce et PrestaShop, SEO complet, maintenance continue sur plusieurs années.",
    features: [
      "E-commerce WooCommerce avec personnalisation produit",
      "Gestion PrestaShop en parallèle",
      "SEO complet et suivi continu",
  ...
  geonomia: {
    ...
    desc: "Site pour un bureau d'études spécialisé en ingénierie environnementale. Rendre des sujets techniques accessibles à un prospect non-expert. Hiérarchie des contenus, appels à l'action B2B, design épuré et crédible.",
    features: [
  ...
  semauri: {
    ...
    desc: "Ma propre agence — l'enseigne sous laquelle j'exerce en freelance. Semauri couvre design, intégration, stratégie éditoriale. Ce portfolio est d'ailleurs une production Semauri.",
    features: [
      "Site WordPress sur mesure + charte graphique",
      "Identité visuelle complète (logo, couleurs, typos)",
      "Design print &amp; digital cohérents",
      "Stratégie communication et référencement",
      "Accompagnement client de A à Z",
    ],
    chips: [
      { label: "WordPress", cls: "fe" }, { label: "Branding", cls: "tool" },
      { label: "UX Design", cls: "tool" }, { label: "SEO", cls: "tool" },
    ],
  },
};
...
function openDescPopup(btn, projectId) {
  ...
  if (!data) return;
  if (activeDescBtn === btn && descPopup.classList.contains("open")) { closeAllDescPopups(); return; }
  activeDescBtn = btn;
...
function closeAllDescPopups() { descPopup.classList.remove("open"); activeDescBtn = null; }
...
(function () {
  ...
  console.log("%c  Front-End Developer · UI Interfaces · O'Clock 2026", s.sub);
  console.log("%c  ──────────────────────────────────────────", s.sub);
  console.log("%c  Tu explores le code ? Super curiosité !", s.sub);
  console.log("%c  HTML · CSS · JS vanilla · Svelte 🚀", s.sub);
  console.log("%c  Interfaces · Performance · Accessibilité", s.sub);
  console.log("%c  ──────────────────────────────────────────", s.sub);
  console.log("%c  Un grand merci aux pirates de O'clock 🏴‍☠️ :", s.sub);
  ...
  console.log("%c  ──────────────────────────────────────────", s.sub);
  console.log("%c  Si tu cherches un dev qui soigne ses interfaces, on discute 😄", s.sub);
})();

/* =============================================
   19. EASTER EGG — GIT DUMBER
   Déclenché au clic sur la card Dev N'Dumber.
   Messages mis à jour v3.1.
============================================= */
(function () {
  const MESSAGES = [
    {
      lines: [
        ...
        '<span class="gdb-ok">✓ Commit envoyé… les pirates de O\'clock approuveraient 🏴‍☠️</span>',
      ],
    ...
    },
    {
      lines: [
        ...
        '<span class="gdb-ok">✓ Merci Anne-so, Ludo, Richard, Juju, Valou, Céd 🏴‍☠️</span>',
    ...
    },
    {
      lines: [
        ...
        '<span class="gdb-fun">😉 Le back c\'est le moteur, le front la jolie carrosserie 🏎️</span>',
      ],
  ...
  document.addEventListener("keydown", (e) => { if (e.key === "Escape" && gdVisible) hideGitDumber(); });
})();

/* =============================================
   20. LIGHTBOX GALERIE GRAPHISME + ONGLETS CATÉGORIES
   
   Encapsulé dans une IIFE pour éviter de polluer le scope global.
   
   collectCategory() : remonte dans le DOM pour trouver le conteneur
   parent et récupérer toutes les images de la même catégorie.
   
   switchGdTab() : affiche le bon panneau en gérant la classe "active".
   
   Navigation tactile : swipe > 50px pour éviter les faux positifs.
============================================= */
(function () {
  ...
  let touchStartX = 0;
  overlay && overlay.addEventListener("touchstart", function (e) { touchStartX = e.touches[0].clientX; }, { passive: true });
